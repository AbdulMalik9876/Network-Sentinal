...
// Cache for geo lookups to avoid hitting rate limits
const geoCache = new Map<string, { country: string; city: string; lat: number; lon: number; isp?: string }>();

/**
 * Look up real geo data for an IP using ip-api.com (free, no key needed, 45 req/min).
 * Returns null for private IPs or on failure.
 */
async function lookupGeoData(ip: string): Promise<{ country: string; city: string; lat: number; lon: number; isp?: string } | null> {
  ...
  try {
    ...
    const data = await resp.json() as { status: string; country?: string; city?: string; lat?: number; lon?: number; isp?: string };
...
export async function generateRealtimeTraffic(): Promise<void> {
  ...
  // -----------------------------------------------------------------
  // STEP 1: Enrich with real geo data from ip-api.com BEFORE storing.
  // This introduces a natural delay (network call) before DB insert.
  // -----------------------------------------------------------------
  ...
  // -----------------------------------------------------------------
  // STEP 2: Now insert enriched event into the database.
  // -----------------------------------------------------------------
  try {
    const [event] = await db.insert(trafficEventsTable).values({
      ...
      lon: geo.lon,
      isSuspicious,
      suspicionReason: isSuspicious ? `Suspicious port ${port} access detected from ${geo.city}, ${geo.country}` : null,
    }).returning();

    // Update device bandwidth
    const byteCount = event.bytes;
    if (direction === "inbound") {
      await db.update(devicesTable).set({ totalBytes: sql`total_bytes + ${byteCount}`, downloadBytes: sql`download_bytes + ${byteCount}`, lastSeen: new Date() }).where(eq(devicesTable.ip, device.ip));
    } else {
      await db.update(devicesTable).set({ totalBytes: sql`total_bytes + ${byteCount}`, uploadBytes: sql`upload_bytes + ${byteCount}`, lastSeen: new Date() }).where(eq(devicesTable.ip, device.ip));
    }
    ...
    if (isSuspicious) {
      ...
      if (settings?.portScanDetection) {
        const inserted = await db.insert(alertsTable).values({
          ...
          message: `Suspicious connection to port ${port} (${protocol}) from ${geo.city}, ${geo.country} [${direction === "inbound" ? remoteIp : device.ip}]`,
          srcIp: direction === "inbound" ? remoteIp : device.ip,
          dstIp: direction === "inbound" ? device.ip : remoteIp,
          port,
        }).returning();
